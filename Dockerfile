...
RUN set -eux; \
    ...
    go build -trimpath -tags release -ldflags="-s -w -X github.com/clawaox-coder/dujiao-next/internal/version.Version=${APP_VERSION}" -o /out/sookauto-api ./cmd/server
...
COPY --from=builder /out/sookauto-api /app/sookauto-api
